...
import { canActivate, canClose, startDeadlinePassed } from "./engine";

export function runScenario(): string[] {
  const now = new Date("2026-09-22T12:00:00Z");
  const p: Permit = {
    id:"SEED-HOT-001", type:"HOT_WORK", location:"TEST-AREA",
    issuedAt:"2026-09-22T09:00:00Z", status:"ISSUED",
    requiresLoto:true, lotoReleased:false,
    requiredGasTest:true, gasTestPassed:true,
    authorizationApproved:true
  };
  const results:string[]=[];
  results.push(canActivate(p) === true ? "PASS: activation guard" : "FAIL: activation guard");
