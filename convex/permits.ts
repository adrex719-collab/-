...
function guardTransition(status: Status, next: Status, p: {authorizationApproved:boolean;requiresLoto:boolean;lotoApplied:boolean;lotoReleased:boolean;requiredGasTest:boolean;gasTestPassed:boolean}) {
  const allowed: Record<Status, Status[]> = {
    REQUESTED:["PENDING_APPROVAL","CANCELLED"], PENDING_APPROVAL:["ISSUED","CANCELLED"],
    ISSUED:["ACTIVE","CANCELLED","EXPIRED"], ACTIVE:["SUSPENDED"], SUSPENDED:["RESUMED","CANCELLED"],
    RESUMED:["CLOSED","SUSPENDED"], CLOSED:[], CANCELLED:[], EXPIRED:[]
  }
...
export const create = mutation({
  ...
  handler:async(ctx,args)=>{
    ...
    const now=Date.now()
    return ctx.db.insert("permits",{...args,status:"REQUESTED",authorizationApproved:false,lotoApplied:false,lotoReleased:false,gasTestPassed:false,createdAt:now,updatedAt:now})
  },
})
...
export const updateState = mutation({
  ...
  handler:async(ctx,args)=>{
    ...
    if(args.lotoReleased&&!permit.lotoReleased&&permit.status!=="RESUMED") invalid("آزادسازی LOTO فقط پس از RESUMED قابل ثبت است.")
